...
typedef struct hiSNAP_PRO_MANUAL_PARAM_S {
    HI_U32 au32ManExpTime[PRO_MAX_FRAME_NUM];/*RW;  Range: [0x0, 0xFFFFFFFF];sensor exposure time (unit: us ), it's related to the specific sensor */
    HI_U32 au32ManSysgain[PRO_MAX_FRAME_NUM];/*RW;  Range : [0x400, 0xFFFFFFFF];system gain (unit: times, 10bit precision) , it's related to the specific sensor and ISP Dgain range */
} SNAP_PRO_MANUAL_PARAM_S;

typedef struct hiSNAP_PRO_AUTO_PARAM_S {
    HI_U16 au16ProExpStep[PRO_MAX_FRAME_NUM];/*RW; Range:[0x0, 0xFFFF];Professional exposure step (unit: times, 8bit precision)*/
...
typedef struct hiSNAP_NORMAL_ATTR_S {
    ...
    HI_U32  u32RepeatSendTimes;  /* [0,2]; When FE-BE is offline, the first raw frame that is triggered should be sent repeatedly to the BE.  */
...
#endif /* End of #ifndef __HI_COMM_SNAP_H__ */
